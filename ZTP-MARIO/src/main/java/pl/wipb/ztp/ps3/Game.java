...
class Game extends JPanel {
    ...
    private List<Segment> stworzPlansze(String plik, Builder builder) {
                    ...
                    switch (znak) {
                        ...
                        case 'A':
                            for (int i = 0; i < liczba; ++i) {
                                builder.createSegment_A(x,y);
                                x += TILESIZE;
                            }
                            break;
                        case 'B':
                            for (int i = 0; i < liczba; ++i) {
                                builder.createSegment_B(x,y);
                                x += TILESIZE;
                            }
                            break;
                        case 'C':
                            for (int i = 0; i < liczba; ++i) {
                                builder.createSegment_C(x,y);
                                x += TILESIZE;
                            }
                            break;
                        case 'G':
                            for (int i = 0; i < liczba; ++i) {
                                builder.createSegment_G(x,y);
                                x += TILESIZE;
                            }
                            break;
                    }
    ...
    public Game(String plik,Builder builder) {
        setPreferredSize(new Dimension(424, 268));
        setFocusable(true);
        plansza = stworzPlansze(plik,builder);
